...
class NoesisEvent:
    ...
    def validate(self) -> None:
        if not self.event_id.strip(): raise ValueError("event_id cannot be empty")
        if self.event_type not in EVENT_TYPES: raise ValueError(f"unsupported event_type: {self.event_type}")
        if not self.source.strip(): raise ValueError("source cannot be empty")
        if self.protocol != PROTOCOL_NAME or self.version != PROTOCOL_VERSION: raise ValueError("unsupported event protocol/version")
        if not isinstance(self.payload, dict): raise ValueError("payload must be an object")
...
def make_event(event_type: str, source: str, payload: dict[str, Any], event_id: str | None = None, timestamp: str | None = None) -> NoesisEvent:
